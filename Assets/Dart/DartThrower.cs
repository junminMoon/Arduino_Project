...
{
    UpdateStatusUI($"Reloading... ({duration:F1}sec)", Color.red);
...
void UpdateGrippingState(bool isDebugging)
{
    ...
    if (touchPressed && !isGripping && isReadyToThrow)
    {
        isGripping = true;
        PrepareDart();
        UpdateStatusUI("ready()", Color.green);
    }
    ...
    void ReloadComplete()
    {
        ...
        else 
        {
            ...
            isGripping = false;
            UpdateStatusUI("Touch!!", Color.white);
        }
    }
}